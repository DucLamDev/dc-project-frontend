...
var EventEmitter = class {
	...
	emit(event) {
		...
		if (this.observers[event]) Array.from(this.observers[event].entries()).forEach((_ref) => {
			let [observer, numTimesAdded] = _ref;
			for (let i = 0; i < numTimesAdded; i++) observer(...args);
		});
		if (this.observers["*"]) Array.from(this.observers["*"].entries()).forEach((_ref2) => {
...
var Translator = class Translator extends EventEmitter {
	...
	exists(key) {
		...
		return this.resolve(key, options)?.res !== void 0;
	}
	...
	translate(keys, options, lastKey) {
		...
		if (handleAsObjectInI18nFormat && resForObjHndl && handleAsObject && noObject.indexOf(resType) < 0 && !(isString(joinArrays) && Array.isArray(resForObjHndl))) {
			...
			if ((usedKey || usedDefault) && this.options.parseMissingKeyHandler) res = this.options.parseMissingKeyHandler(this.options.appendNamespaceToMissingKey ? `${namespace}:${key}` : key, usedDefault ? res : void 0);
		}
		if (returnDetails) {
			resolved.res = res;
			resolved.usedParams = this.getUsedParamsDetails(options);
